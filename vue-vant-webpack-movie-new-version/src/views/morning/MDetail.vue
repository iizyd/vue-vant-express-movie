<template>
    <div>
        ...
        <div class="set-list-box">
            <div class="play-source" v-for="(i, j) in set_list" :key="j">
                <div class="source-title">{{i.set_source}}</div>
                <div
                    class="set-list-item"
                    v-for="(item, index) in i.sets"
                    :key="index"
...
<script>
import { MgetDetailData, MgetPlayUrl } from '../../utils/api_methods'
...
export default {
    ...
    created () {
        ...
        MgetDetailData({ params: { url: _href } }).then(res => {
            console.log(res);
    ...
    methods: {
        ...
        play (item) {
            ...
            MgetPlayUrl({ params: { url: item.set_href } }).then(res => {
                console.log(res.data);
                this.iframe_src = res.data.src;
                this.loadingTag.clear();
            })
        },
        changeBgColor (item) {
